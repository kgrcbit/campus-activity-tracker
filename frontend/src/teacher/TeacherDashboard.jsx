...
import { Filter, Download, User, Calendar, Table2, CheckCircle, XCircle, Clock, FileText, Loader2, School } from 'lucide-react';
...
const TeacherDashboard = () => {
    ...
    const loadSubmissions = useCallback(async () => {
        ...
        try {
            // For teachers, use the teacher-specific endpoint to get only their class submissions
            const endpoint = `${process.env.REACT_APP_API_URL}/submissions/teacher`;

            const response = await API.get(endpoint, { params: apiFilters });
            setSubmissions(response.data.data || response.data);
            console.log('Loaded teacher submissions:', response.data);
        } catch (error) {
...
const response = await axios.get(`${process.env.REACT_APP_API_URL}/templates`);
    ...
    const handleExport = async (format) => {
        ...
        try {
            ...
            // For teachers, use the teacher-specific export endpoint
            const endpoint = `${process.env.REACT_APP_API_URL}/submissions/teacher/export`;
            ...
            link.href = url;
            const filename = `Teacher_Class_Report_${format}_${new Date().toISOString().split('T')[0]}.${format.toLowerCase()}`;
    ...
    const columns = [
        ...
        { header: 'Section', accessor: 'userId', render: (user) => user?.section || 'N/A' },
    ...
    return (
        <div className="admin-container">
            ...
            <header className="header-card">
                ...
                <p className="header-subtitle">View and manage submissions from your assigned students.</p>
                <p className="header-subtitle">
                    Class: {useAuthStore.getState().user?.Year} - {useAuthStore.getState().user?.assignedSection}
                </p>
            </header>
            ...
            <div className="table-card">
                <h2 className="table-header">Student Submissions ({submissions.length})</h2>
                {isLoading ? (
